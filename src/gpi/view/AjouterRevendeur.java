...
public class AjouterRevendeur {
	...
	/**
	 * Cette procedure permet de fermer la fenetre et d'ajouter le revendeur
	 * dans la BD lorsque le bouton AJOUTER est clique
	 */
	@FXML
	private void handleOk() {
		if (controlerSaisies() == true) {
			Revendeur revendeurAAjouter = new Revendeur(
					new SimpleIntegerProperty(0), tf_nomRevendeur.getText(),
					tf_telRevendeur.getText(), tf_mobileRevendeur.getText(),
					tf_faxRevendeur.getText(), tf_emailRevendeur.getText(),
